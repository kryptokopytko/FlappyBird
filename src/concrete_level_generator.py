...
from entities.coin import Coin, REGULAR_COIN_VALUE, GOLD_COIN_VALUE
            ...
            if i not in self.generated_items:
                if abs(item_data.x - pipe.x) < 20:
                    ...
                    if item_data.type == "coin":
                        value = GOLD_COIN_VALUE if item_data.is_gold else REGULAR_COIN_VALUE
                        item = Coin(x=item_data.x, y=item_data.y, value=value)
                    elif item_data.type == "powerup":
                        item = PowerUp(x=item_data.x, y=item_data.y, powerup_type="speed")
                    elif item_data.type == "debuff":
                        item = PowerUp(x=item_data.x, y=item_data.y, powerup_type="slow")
                    else:
    ...
    def generate_items(self, scroll_offset: float) -> List:
        ...
        for i, item_data in enumerate(self.concrete_level.items):
            ...
            if view_start <= item_data.x <= view_end and i not in self.generated_items:
                # Create appropriate game entity
                if item_data.type == "coin":
                    item = Coin(x=item_data.x, y=item_data.y, is_gold=item_data.is_gold)
                elif item_data.type == "powerup":
                    item = PowerUp(x=item_data.x, y=item_data.y, powerup_type="speed")
                elif item_data.type == "debuff":
                    item = PowerUp(x=item_data.x, y=item_data.y, powerup_type="slow")
