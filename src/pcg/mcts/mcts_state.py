...
from entities.powerup import POWERUPS
...
class MCTSState:
    ...
    def copy(self) -> "MCTSState":
        return MCTSState(
            ...
            items=[Item(i.x, i.y, i.type) for i in self.items],
...
def apply_action(state: MCTSState, action: MCTSAction) -> MCTSState:
    ...
    if 'coin' in action.item_config:
        ...
        new_state.items.append(Item(coin_x, coin_y, 'coin'))
    ...
    if 'powerup' in action.item_config:
        ...
        new_state.items.append(Item(powerup_x, powerup_y, random.choice(POWERUPS)))
